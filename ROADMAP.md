...
- [ ] **B6-P2 — Safety events feed de Samsara** (spec: `docs/ROADMAP.md` §4 B6-P2, esfuerzo L — partir en poller/normalización si crece; depende de P1 ya cerrado). Sin tablas nuevas: `PollSamsaraSafetyEventsJob` (cola `ingestion`, scheduler 1–2 min `onOneServer()`, patrón `PollAllAssetLocationsJob`) lee `GET /safety-events/stream` paginando con cursor persistido en `sync_state_json` (arranque sin cursor = `now()-24h`); cada evento → `RawEvent` (`event_type_raw='SafetyEvent'`, dedup key `safety:{id}:{eventState}`); media inline descargada al momento → `RawEventAttachment`; seeders de `EventMappingRule`/`EventType` por behaviorLabel (Crash→collision critical, Drowsy→driver_fatigue high, MobileUsage→distracted_driving medium, SevereSpeeding→severe_speeding high); `eventState=dismissed` → `ApplyExternalResolution`; `RecordUsageEvent` por evento. Tests: cursor avanza/no repite/retoma, mapeos por label, update de estado no duplica incidente, media inline → `EventMediaContext`, tenant isolation.
