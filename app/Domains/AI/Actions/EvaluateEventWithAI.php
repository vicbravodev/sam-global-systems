        ...
        if ($rulesDecision !== null) {
            return DB::transaction(function () use ($event, $version, $rulesDecision, $riskScore, $input) {
                return $this->persistEvaluation(
                    ...
                    confidence: 0.95,
                    riskScore: $riskScore,
                    explanationSummary: 'Short-circuited by deterministic rule: '.$rulesDecision['reason'],
        ...
        if ($this->quotaExceeded($event->team_id, $profile->monthlyTokenLimit)) {
            return DB::transaction(function () use ($event, $version, $riskScore, $input) {
                return $this->persistEvaluation(
                    ...
                    confidence: 0.5,
                    riskScore: $riskScore,
                    explanationSummary: 'Tenant AI quota exhausted; falling back to rules_only.',
        ...
        try {
            ...
            return DB::transaction(function () use ($event, $version, $riskScore, $input, $exception) {
                return $this->persistEvaluation(
                    ...
                    confidence: 0.4,
                    riskScore: $riskScore,
                    explanationSummary: 'AI agent failed; fallback to rules_only. Error: '.$exception->getMessage(),
