...
| 08 | Context (core) | ✅ PR #1 | `tests/Feature/Domains/Context/*`, `tests/Unit/Domains/Context/Support/*` | Snapshots, geofences, perfil operacional, `EnrichContextJob`. `GetRelatedOpenIncidents` ya consulta el modelo `Incident` real. **Pipeline de media diferido a spec 08 PR #2.** |
...
| Contracts y Null impls referenciaban `SPEC-XX-DEFERRED` para specs que ya shipearon | Comentarios reformulados; los Null restantes (Decision/Incident metrics, Audit log query) sólo se mantienen porque aún no existe una query DB-backed real. | `app/Contracts/Decisions/DecisionMetricsQuery.php`, `app/Contracts/Incidents/IncidentMetricsQuery.php`, `app/Contracts/Audit/AuditLogQuery.php`, sus Null impls. |
...
- **Spec 08 PR #2** — pipeline de media: `EventMediaAsset`, `ExtractEventMediaJob`, `EventRelatedIncidentLink`. Depende de FileObject (I1) + Samsara media endpoints.
...
- **`SPEC-09-MULTIMODAL-DEFERRED`** (spec 09 → depende de spec 08 PR #2): `ai_media_assessments`, `EvaluateEventMultimodally`, `EvaluateEventMediaJob`.
...
- **Decision / Incident / Audit metrics queries** — los contratos `DecisionMetricsQuery`/`IncidentMetricsQuery`/`AuditLogQuery` siguen bindeados a Null hasta que cada dominio dueño exponga una query DB-backed para Analytics.
