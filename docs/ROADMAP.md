...
> Documento vivo de next steps para frontend y backend. Actualizado al **2026-06-09** (PR #50, B3 cerrado; + plan B6 pipeline de emergencias P1–P8), verificado contra el código (no contra docs viejas).
...
| Pipeline de emergencias (panic) | Auditoría 2026-06-09 del flujo panic end-to-end: el incidente crítico se crea confiablemente, pero `isResolved` se guarda en normalización y nadie lo lee (los fixtures reales traen `isResolved: true` y aun así abrirían incidente); sin GPS fresco (se usa `latestLocation`, posiblemente stale); sin media on-demand aunque `has_camera` está en el snapshot y `EventMediaRequest`/`FetchDeferredEventMediaJob` existen (stub sin adapter); SLA decorativo (`response_sla_seconds=300` sin timer ni escalación); notificación genérica sin ubicación ni asignación de operador; panics del mismo vehículo fuera de la ventana de 30 min quedan sin vínculo. **Plan completo en §4 B6 (P1–P8).** |
...
#### P1 🔵 EN CURSO — Ciclo de vida de `isResolved` (quick win)
`is_resolved` se guarda en `NormalizeRawEvent` (línea ~188) y nadie lo consume. Trampa incluida: un reenvío del `AlertIncident` con `isResolved: true` hoy muere en dedup por `eventId`. Diseño: (a) la clave de dedup de `AlertIncident` incorpora el estado de resolución para dejar pasar cambios de estado (mismo estado sigue siendo duplicado); (b) acción `Incidents/ApplyExternalResolution` — anota timeline "resuelto en origen" + `external_resolved_at` en `incidents`; (c) `TenantSetting` `panic.auto_close_on_external_resolution` = `annotate` (default) | `close`; (d) si el evento ya llega resuelto, el incidente se crea igual pero con la anotación visible y la señal disponible para P7. Tests: cambio de estado no es duplicado / mismo estado sí; auto-close solo con setting; tenant isolation. **Esfuerzo: S. Un agente ya está trabajando este ítem (2026-06-09) junto con el trigger de media de P3.**
...
#### P8 — Vínculo histórico de incidentes
`GetRelatedOpenIncidents` solo mira la ventana de 30 min; añadir lookup de incidentes **cerrados** del mismo asset/driver en 7 días → `EventRelatedIncidentLink` tipo nuevo `PriorSimilarIncident`. Entra al snapshot de contexto (la IA ve "tercer panic de este camión esta semana") y a la UI del incidente como "Historial relacionado". Sin merge automático — solo visibilidad. **Esfuerzo: S.**

**Orden recomendado dentro de B6:** P1 (en curso) → P2 → P3 → P4 → P5 → P6 → P7 → P8. Un PR por fase (P2 puede partirse en poller/normalización si crece), CI verde, y actualizar este documento en el mismo PR que cierre cada P.
...
| **3. Flota visible + emergencias 🔵 ACTUAL** | F3 (assets + mapa) → F4 (drivers) · backend en paralelo: B6-P1 🔵 (isResolved, en curso) → P2 (safety feed) → P3 (media panic) → P4 (GPS fresco) | El operador ve su flota en vivo y el pipeline de emergencias consume señales reales (resolución, footage, posición fresca, safety events) |
