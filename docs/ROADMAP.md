...
- Señales nuevas: `nearby_safety_events_count`, `harsh_driving_near_event` (bool si hay frenada/maniobra brusca en la ventana); facts equivalentes en el motor de reglas y entrada explícita en el prompt del `EventClassifierAgent` ("un pánico con frenada brusca 2 min antes pesa hacia evento real").
- Persistir en `recent_history_snapshot_json` para que el detalle del incidente lo muestre.
